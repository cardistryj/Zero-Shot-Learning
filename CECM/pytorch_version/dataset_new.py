...
class DataSet(object):
    ...
    def loadDataSet(self, path, unit_num):
        ...
        for modulation in range(1,12):

            for snr in data_name:
                data=io.loadmat(snr)
    
                raw_train=data['rxTraining']
                raw_label=data['rxTrainingLabel']

                raw_train=raw_train.transpose((3,2,0,1))


                single_data=raw_train[np.where(raw_label==modulation)]
...
#        print(np.max(self.X))
    ...
    def splitData(self, rate=0.8, num_seen_class = 8, seed=2019):
        ...
        self.Y_train = self.to_onehot(list(map(lambda x: train_class.index(self.lbl[x]), self.train_idx)))
        self.Y_test = self.to_onehot(list(map(lambda x: train_class.index(self.lbl[x]), self.test_idx)))
